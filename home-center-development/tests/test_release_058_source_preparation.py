...
def test_release_058_runtime_contracts_are_packaged() -> None:
    assert {
...
def test_release_058_ci_derives_candidate_artifact_name_from_version() -> None:
...
def test_release_058_upgrade_drills_do_not_rewrite_candidate_identity() -> None:
    for name in (
        ...
        assert "_build_exact_artifact" in source
