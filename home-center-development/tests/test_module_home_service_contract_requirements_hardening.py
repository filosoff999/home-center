...
def test_contract_count_boundary_is_bounded_to_schema_limit() -> None:
    accepted = tuple(
        f"service.contract{index}.v1" for index in range(64)
    )
    assert len(
        _build(required_service_contracts=accepted).required_service_contracts
    ) == 64
    with pytest.raises(
        ModuleHomeServiceContractRequirementError,
...
def test_unbounded_contract_iterable_is_consumed_only_to_limit_plus_one() -> None:
    consumed = 0
...
def test_iterator_failure_is_normalized_to_stable_rejection() -> None:
...
def test_polymorphic_scalar_and_serialized_containers_are_rejected() -> None:
