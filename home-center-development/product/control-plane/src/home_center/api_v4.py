...
_REAUTH_SCOPE = re.compile(
    r"^(?:household\.device\.management\.enrollment\.verify:dmpverify-[0-9a-f]{24}"
    r"|household\.device\.management\.deenrollment:dmdel-[0-9a-f]{24})$"
)


def _reauth_scope_allowed(scope: object) -> bool:
    return isinstance(scope, str) and _REAUTH_SCOPE.fullmatch(scope) is not None
...
class RuntimeRequestHandlerV4(RuntimeRequestHandlerV3):
    ...
    def _deenrollment(self, path: str) -> None:
        self._request_body_complete = False
        correlation_id = self._correlation_id()
        context = self._classify_request(correlation_id)
        if context is None:
        ...
        try:
            body = self._read_json(max_bytes=8192)
            service = self.runtime.device_management_deenrollment
            if path.endswith("/plan"):
                value = service.plan(actor=actor, request=body, correlation_id=correlation_id)
            else:
                value = service.confirm(
            ...
            self._json(HTTPStatus.OK, value)
            return
        except DeviceManagementDeenrollmentRuntimeError as exc:
            forbidden = {
                "household_actor_not_bound",
                "household_member_disabled",
            ...
            unavailable = {"household_state_invalid"}
            if exc.code in forbidden:
                status = HTTPStatus.FORBIDDEN
            elif exc.code in not_found:
                status = HTTPStatus.NOT_FOUND
            ...
            self.runtime.store.audit(
                actor=actor,
                action="household.device.management.deenrollment.request",
                target="household",
                outcome="denied",
                correlation_id=correlation_id,
                details={"reason": exc.code, "path": path},
            ...
            self._error(
                status,
                exc.code,
                "Запрос отключения управления устройством не прошёл безопасную проверку",
                correlation_id,
            )
        except (ValueError, TypeError, json.JSONDecodeError):
            self.runtime.store.audit(
                ...
                correlation_id=correlation_id,
                details={
                    "reason": "invalid_device_management_deenrollment_request",
                    "path": path,
                },
            )
            self._error(
    ...
    def _reauth(self) -> None:
        ...
        try:
            ...
            if (
                ...
                or not _reauth_scope_allowed(scope)
            ):
