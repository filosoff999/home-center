...
Home Center 0.57.0 предназначен для Linux с systemd, Python 3.12 или новее и SQLite. TLS-идентичности для Web и межузлового взаимодействия предоставляет администратор.

## Проверка релиза

Перед установкой проверьте опубликованный tag `v0.57.0`, `SHA256SUMS`, release manifest, acceptance evidence и SPDX SBOM. Контрольная сумма подтверждает целостность файла, но сама по себе не подтверждает совместимость конкретного upgrade/updater path.

Для **чистой установки 0.57.0** используйте официальный source-архив Stable-релиза и штатный скрипт:
