...
class Release015QualificationTests(unittest.TestCase):
    def test_release_identity_is_consistent(self) -> None:
        project = tomllib.loads((ROOT / "pyproject.toml").read_text(encoding="utf-8"))["project"]
        release_notes = (ROOT / "RELEASE.md").read_text(encoding="utf-8")
        self.assertEqual(project["version"], RELEASE)
        self.assertEqual(home_center.__version__, RELEASE)
        self.assertIn(f"# Release {RELEASE}", release_notes)
