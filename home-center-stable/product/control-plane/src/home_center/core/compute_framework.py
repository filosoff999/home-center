"""Side-effect-free compute provider planning for Home Center 0.14."""
...
class ComputeResourceRequest:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            not isinstance(self.memory_mib, int)
            or isinstance(self.memory_mib, bool)
            or not 256 <= self.memory_mib <= 4_194_304
        ):
...
class ComputePlanner:
    """Validate provider/capacity constraints and return a plan only."""
    ...
    def plan_create(
        ...
        *,
        ...
        for value, code in (
            (available_cpu, "invalid_available_cpu"),
            (available_memory_mib, "invalid_available_memory"),
            (available_storage_gib, "invalid_available_storage"),
        ):
        ...
        required_capability = self._RUNTIME_CAPABILITY[request.kind]
