"""Secret-free certificate lifecycle inventory and renewal planning for Home Center 0.14."""
...
class CertificateLifecyclePlanner:
    """Plan renewal only when identity/trust/maintenance preconditions are known."""
    ...
    def plan_renewal(
        ...
        *,
        ...
        if not isinstance(issuer_available, bool) or not isinstance(service_reload_supported, bool):
            raise CertificateLifecycleError("invalid_certificate_preflight")
        ...
        return CertificateRenewalPlan(
            certificate_id=record.certificate_id,
            service_id=record.service_id,
            status=status,
            state=CertificatePlanState.BLOCKED if blockers else CertificatePlanState.PLANNED,
