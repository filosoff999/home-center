...
def main() -> None:
    ...
    require('[ "$VERSION" = 0.9.1 ] || { echo RELEASE_VERSION_NOT_ADMITTED' in build, "0.9 successor artifact version is not admitted exactly")
    require("HOME_CENTER_080_ARTIFACT_NOT_YET_ADMITTED" not in build, "obsolete 0.8 artifact block remains")
    require("HOME_CENTER_091_CONFIG_SCHEMA_NOT_ADMITTED" in build, "0.9 config-v4 release gate missing")
