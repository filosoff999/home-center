#!/usr/bin/env python3
"""Real-Chrome acceptance for an immutable Home Center 0.7.0 candidate.
...
VALID_TOKEN = "candidate-browser-fixture-token-000000000000000000000"
COOKIE_VALUE = "candidate-070-browser-session"
                ...
                self._json(
                    401,
                    {
                        "error": {
                            ...
                            "message": "SENSITIVE TOKEN BACKEND DETAIL MUST NEVER BE RENDERED",
                        }
                    },
                )
            return
...
def browser_run(
    ...
    *,
    ...
    try:
        ...
        driver.clear("#tokenInput")
        driver.type("#tokenInput", VALID_TOKEN)
        driver.click("#loginForm button[type='submit']")
        wait_for(lambda: overlay(driver)["display"] == "none", f"{mode} successful token login")
...
def main() -> int:
    args = arguments()
    if args.expected_version != "0.7.0":
        raise RuntimeError("browser gate only admits Home Center 0.7.0")
    ...
    if results.failures:
        print("HC_070_BROWSER_ACCEPTANCE=FAIL")
        return 1
    print("HC_070_BROWSER_ACCEPTANCE=PASS")
    return 0
