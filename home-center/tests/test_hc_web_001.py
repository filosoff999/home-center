...
class HCWeb001RegressionTests(unittest.TestCase):
    ...
    def test_module_lifecycle_plan_is_blocked_and_has_no_execution_control(self) -> None:
        ...
        self.assertIn('step.publication?.status === "published"', javascript)
        self.assertIn('step.publication?.installation_authority === false', javascript)
        self.assertIn('step.publication?.artifact?.content_address === `sha256:${step.module?.artifact_sha256}`', javascript)
        self.assertIn("Публикация не подтверждена", javascript)
