...
`deploy/` содержит декларативные profiles и versioned fail-closed scripts поставки Home Center. Production baseline принят на `0.3.0`; изменения Web PKI в scripts относятся к `0.4.1` release candidate до отдельной acceptance. Канонический двухузловой rollout выполняется immutable artifact в порядке `dc02 → dc01`; каждый node install создаёт rollback point и проверяет readiness, helper, timers и backup.
