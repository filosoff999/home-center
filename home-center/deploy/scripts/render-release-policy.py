...
BOOTSTRAP_REPLACEMENTS = {
    '[ "$TARGET_VERSION" = 0.5.0 ] || { echo RELEASE_VERSION_NOT_ADMITTED >&2; exit 66; }':
        '[ "$TARGET_VERSION" = 0.7.0 ] || { echo RELEASE_VERSION_NOT_ADMITTED >&2; exit 66; }',
    "ADMITTED_SOURCE_V043_VERSION=0.4.3": "ADMITTED_SOURCE_V060_VERSION=0.6.0",
    "ADMITTED_SOURCE_V043_REVISION=64f798ceae0b669cbac01b452c3cf4fd96070136":
        f"ADMITTED_SOURCE_V060_REVISION={SOURCE_REVISION}",
    "ADMITTED_SOURCE_V043_RELEASE=/opt/home-center/releases/0.4.3-64f798ceae0b-b2dde6a51ec9":
        f"ADMITTED_SOURCE_V060_RELEASE={SOURCE_RELEASE}",
    '  [ "$version" = "$ADMITTED_SOURCE_V043_VERSION" ] \\':
        '  [ "$version" = "$ADMITTED_SOURCE_V060_VERSION" ] \\',
    '    && [ "$revision" = "$ADMITTED_SOURCE_V043_REVISION" ] \\':
        '    && [ "$revision" = "$ADMITTED_SOURCE_V060_REVISION" ] \\',
    '    && [ "$release" = "$ADMITTED_SOURCE_V043_RELEASE" ]':
        '    && [ "$release" = "$ADMITTED_SOURCE_V060_RELEASE" ]',
}
...
def render(bootstrap_path: Path, install_path: Path) -> None:
    ...
    required_bootstrap = (
        '[ "$TARGET_VERSION" = 0.7.0 ]',
        "ADMITTED_SOURCE_V060_VERSION=0.6.0",
        f"ADMITTED_SOURCE_V060_REVISION={SOURCE_REVISION}",
        f"ADMITTED_SOURCE_V060_RELEASE={SOURCE_RELEASE}",
    )
    ...
    if '[ "$VERSION" = 0.7.0 ]' not in installer or '[ "$VERSION" = 0.6.0 ]' in installer:
