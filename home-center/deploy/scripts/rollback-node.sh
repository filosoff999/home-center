...
restore_optional() {
  ...
  if [ -f "$POINT/$name.existed" ]; then
    install -m 0644 -o root -g root "$POINT/$name" "$target"
  else
    rm -f "$target"
  fi
}
restore_optional helper-policy.json /etc/home-center/helper-policy.json
restore_optional home-center-helper.service /etc/systemd/system/home-center-helper.service
restore_optional home-center-tls-maintenance.service /etc/systemd/system/home-center-tls-maintenance.service
restore_optional home-center-tls-maintenance.timer /etc/systemd/system/home-center-tls-maintenance.timer
...
if [ -n "$PREVIOUS" ]; then
  ...
  if [ -f /etc/systemd/system/home-center-tls-maintenance.timer ] && [ -f "$PREVIOUS/home_center/tls_maintenance.py" ]; then
    systemctl enable --now home-center-tls-maintenance.timer >/dev/null
