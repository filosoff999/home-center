...
- `modules/module-artifact-publication.v1.schema.json` — immutable verified publication evidence with logical content address and no installation authority;
- `modules/module-admission-request.v1.schema.json` — exact local environment, installed-module constraints, candidate manifests and requested identities;
- `modules/module-admission-result.v1.schema.json` — deterministic dependency-first admission plan with requested permissions and production activation fixed off;
- `modules/module-permission-review.v1.schema.json` — deterministic read-only attribution of permissions to exact admitted modules and typed actions;
- `modules/module-permission-review-status.v1.schema.json` — honest no-pending-review state before a Market selection exists;
- `modules/module-permission-acknowledgement-request.v1.schema.json` — explicit actor-bound acknowledgement input carrying the exact admission request and review identity;
- `modules/module-permission-acknowledgement.v1.schema.json` — expiring persisted evidence with a stable but non-consumable lifecycle handoff reference;
- `modules/module-permission-acknowledgement-result.v1.schema.json` — create/replay envelope;
- `modules/module-permission-acknowledgement-list.v1.schema.json` — actor-scoped recent history;
- `modules/module-install-lifecycle-request.v1.schema.json` — exact admission request plus actor-owned acknowledgement reference for install planning;
- `modules/module-install-lifecycle-plan.v1.schema.json` — dependency-first blocked steps, exact server-side publication evidence, explicit preflight blockers and reverse-order recovery;
- `modules/module-lifecycle-status.v1.schema.json` — honest no-pending-lifecycle state before a Market selection exists;
- `openapi/home-center-modules.v1.openapi.json` — authenticated review/acknowledgement/lifecycle status and same-origin preview surfaces.

`ModuleManifest v2` is descriptive and production-inert. It grants no installation, download, service-control, filesystem-path, generic command or privileged-helper authority. Schema validation is necessary but not sufficient: strict JSON, cross-field invariants, DSSE signatures, normalized manifest binding, archive safety and exact content-addressed staging are enforced by `home_center.module_manifest` and `home_center.module_artifact`. Package acquisition, extraction and lifecycle execution remain disabled. `home_center.module_admission` additionally validates local compatibility, dependencies and conflicts without I/O, placement, permission grants or lifecycle authority. `home_center.module_permission_review` recomputes admission, binds exact artifacts, classifies permissions fail-visibly and produces no decision, grant, install or activation authority. `home_center.module_permission_acknowledgement` persists only a 15-minute HMAC-protected record of explicit review acknowledgement; its handoff reference is non-consumable and grants no authorization, permission, lifecycle or production authority. `home_center.module_artifact_publication` records HMAC-protected evidence only after verified content-addressed staging and exposes no acquisition or publication HTTP route. `home_center.module_lifecycle` loads that evidence server-side, binds it to the exact full admission request and emits only a blocked install plan with declared health/backup/recovery metadata. Artifact mutation inside lifecycle, placement, authorization, lifecycle persistence/execution and production activation remain disabled.
