...
After successful authentication the password is discarded and the browser receives only a short-lived HMAC-signed session cookie with `Secure`, `HttpOnly` and `SameSite=Strict`. Authenticated actors use the canonical form `local-admin:<username>` for RBAC and audit attribution.
...
The runtime configuration schema advances to `home-center.config.v2` and replaces `admin_token_file` with `local_admin_credentials_file`.
